<section class="section container-fluid">
  <div class="row">
    <div class="col-md-12 col-lg-3">
      <div class="sectionHeading">RECENT NEWS</div>
      <div class="h-section card sectionContent container">
        <div class="card-body">
                    <marquee behavior="scroll" scrolldelay=60 scrollamount=4 direction="up" height=100%>
          <ul class="list-group list-group-flush text-primary">
            <li class="list-group-item">Mahatma Gandhi Birthday Celebration Conducted on 2 October At JECRC</li>
            <li class="list-group-item">Department of Mathematics organized an International conference on Mathematical Modeling & Computing on April, 6-7 2018 at JECRC.</li>
          </ul>
          </marquee>
        </div>
      
    </div>
      </div>

    <div class="col-md-12 col-lg-6">
      <div class="sectionHeading">NEWS LETTER - Applied Science</div>
      <div class="section container card h-section">
        <div class="sectionContent card-body">
          <div class="owl-carousel owl-theme py-2 px-2">
    <div class="item"><a href="../departments/as/src/pdf/asnews1.pdf" download><img src="../departments/as/src/img/news/asnews1.png" alt="" class="img-thumbnail"></a></div>
   
    
</div>
 <p class="px-2 pt-2">
           

          </p>
        </div>
      </div>
    </div>
  </div>
</section>





<script>
  $('.owl-carousel').owlCarousel({
    loop:false,
    margin:10,
    nav:true,
    items: 3,
    center: true
    
})
</script>